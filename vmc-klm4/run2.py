...
if a proposed configuration was rejected, i will drop it,
not consider it
@author: aa
"""
...
def ele_hop(conf_new,L,mc,md,v):
    while True:
        ...
        if conf_new[0][s_conf_index] !=0 and conf_new[0][hop_conf_index]==0:
            ratio=Ratio(conf_new,s_conf_index,hop_conf_index,L,j,mc,md,v)[0]
            rho=ratio**2
...
def spin_flip(conf,L,mc,md,v):
    while True:
        ...
        if conf_new[0][s_conf_index] !=0 and conf_new[0][(s//4)*8+s%4]==0 and conf_new[0][(s//4)*8+(s%4+2)%4] !=0:
            ...
            ratio=Ratio(conf_new,(s//4)*8+4+s%4,(s//4)*8+4+(s%4+2)%4,L,j,mc,md,v)[0]*Ratio(conf_temp,(s//4)*8+(s%4+2)%4,(s//4)*8+s%4,L,j,mc,md,v)[0]
            rho=ratio**2
            if random.uniform(0,1)<min(1,rho):
