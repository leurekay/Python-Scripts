# -*- coding: utf-8 -*-
"""
Created on Sun Oct 23 16:08:23 2016
j固定，遍历变分参数：mc,md,v
@author: aa
"""
...
def get_observable(L,j,mc,md,v):
    conf_new=conf_initial(L)
    conf_total=[conf_new.copy()]
    box=[]
    samples=0
    ...
    f.close()
    return [E_cell,(OmcA+OmcB)/2,(OmdA+OmdB)/2,(Ov+Ov1)/2]

mc_range=np.arange(0.1,0.8,0.15)
md_range=np.arange(0.1,0.8,0.15)
v_range=np.arange(0.3,1.4,0.15)

for mc in mc_range:
    for md in md_range:
        for v in v_range:
            E_cell,Omc,Omd,Ov=get_observable(L,j,mc,md,v)
            f = open('C:/Users/aa/Documents/Python Scripts/vmc-klm5/j='+str(j)+'.txt', 'a+')
            f.write("E_cell=%.2f Omc=%.2f Omd=%.2f Ov=%.2f  (%.2f-%.2f-%.2f)"%(E_cell,Omc,Omd,Ov,mc,md,v)+"\n\n")
            f.close()
