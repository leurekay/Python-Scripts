...
def ele_hop(G,Gindex,conf_new,L,mc,md,v,u):
    ...
    #GGinv=np.dot(G,Ginv)   #test
    if random.uniform(0,1)<min(1,rho):
        #print "updated" #test0
        conf_new[0][s_conf_index]=0
        conf_new[0][hop_conf_index]=1 
        SIGMA.resize((4*L**2,1))
        Gbeta=G[beta,:]
        Gbeta.resize((1,4*L**2))
        G_new=G+(np.dot(np.dot(G,SIGMA),Gbeta))/ratio
        #global W #test
        #W=np.dot(G,SIGMA)  #test
...
def spin_flip1(G,Gindex,conf,L,mc,md,v,u):   
    #dyson chain update
    while True:
    ...
    if conf[0][same_ele]==0 and conf[0][opposite_ele] !=0:
        ...
        u=np.conjugate(u.T)
        beta=Gindex.index(s_conf_index)     
        M=u[np.arange(0,L*L*4,1),:]
        SIGMA=M[:,s_conf_index]-M[:,same_ele]
        ratio1=1-np.dot(G[beta,:],SIGMA)   
        conf_temp=conf.copy()
        ...
        Gbeta=G[beta,:]
        Gbeta.resize((1,4*L**2))
        G_temp=G+(np.dot(np.dot(G,SIGMA),Gbeta))/ratio1
        Gindex_temp=copy.copy(Gindex)    
        Gindex_temp[beta]=same_ele  
            
        beta=Gindex.index(opposite_ele)     
        SIGMA=M[:,opposite_ele]-M[:,opposite_spin]
        ratio2=1-np.dot(G_temp[beta,:],SIGMA)  
        ratio=ratio1*ratio2
        rho=ratio.conjugate()*ratio
        rho=rho.real
        u=np.conjugate(u.T)
        if random.uniform(0,1)<min(1,rho):
            ...
            Gbeta=G[beta,:]
            Gbeta.resize((1,4*L**2))
            G_new=G_temp+(np.dot(np.dot(G_temp,SIGMA),Gbeta))/ratio2
            Gindex_temp[beta]=opposite_spin  
            return ratio,conf_temp,G_new,Gindex_temp
        else:
            return "reject"
...
while Nwarmup:
    ...
    for i in range(2*L**2):
        try_spin=spin_flip1(G,Gindex,conf_new,L,mc,md,v,u)
        if try_spin != "reject":
            ratio,conf_new,G,Gindex=try_spin
    if count %kedu ==0:
