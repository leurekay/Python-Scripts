# -*- coding: utf-8 -*-
"""
Created on Sun Nov 06 16:46:51 2016
这个版本有问题，大错特错
@author: aa
"""
...
Nwarmup=1000
Nstep=1000
LOOP=2                  #for LOOP monte-carlo-step,do measure
...
def Dyson_update(conf,before,after,L,j,mc,md,v,u):
    global G
    global G_index
    
    u=u.T
    beta=G_index.index(before)
    copy_G=copy.copy(G) 
    copy_index=copy.copy(G_index)        
    M=u[np.arange(0,L*L*4,1),:]
    
    SIGMA=M[:,before]-M[:,after]
    den=1-np.dot(copy_G[beta,:],SIGMA)
    G_new=copy_G+np.dot(np.dot(copy_G,SIGMA),copy_G[beta,:])/den
    copy_index[beta]=after
    return den,G_new,copy_index
...
def ele_hop(conf_new,L,mc,md,v,u):
    ...
    ratio,G_temp,G_index_temp=Dyson_update(conf_new,s_conf_index,hop_conf_index,L,j,mc,md,v,u)    
   
    rho=ratio.conjugate()*ratio
    if random.uniform(0,1)<min(1,rho):
        G=G_temp
        G_index=G_index_temp
...
def warm_up(Nwarmup,conf,L,mc,md,v,u):
    ...
    while Nwarmup:
        count=count+1
        Nwarmup=Nwarmup-1
        G,G_index=G_and_Idx(conf,L,j,mc,md,v,u)
...
Esquare_total3=0
conf_new=conf_initial(L)
G,G_index=G_and_Idx(conf_new,L,j,mc,md,v,u)
